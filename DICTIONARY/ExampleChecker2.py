...
# Function to check that every example \xv has corresponding \xe, \xn, and \xf translations in the next three lines
def check_example_translations(file_path):
    ...
    while line_number < len(lines):
        ...
        if re.match(r"\\xv", line):
            ...
            # Look for \xe, \xn, and \xf in the next three lines
            for i in range(1, 4):
                if line_number + i < len(lines):
                    next_line = lines[line_number + i].strip()
