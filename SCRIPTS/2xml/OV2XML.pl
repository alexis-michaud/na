...
# Declaring variables
my $vocinfile='E:\24_naxi\Yongning\F4_LD_AMA\F4_tons\fichiers_archives\SubjectAndVerb\Tone_Subject_Verb_F4_2012.txt';
#my $vocinfile = 'E:\24_naxi\Yongning\F4_LD_AMA\F4_tons\fichiers_archives\SubjectAndVerb\test.txt';
my $regionsinfile = 'E:\24_naxi\Yongning\F4_LD_AMA\F4_tons\fichiers_archives\SubjectAndVerb\Tone_Subject_Verb_F4_2012_REGIONS.txt'; 
...
open (XOUT,">E:\\24_naxi\\Yongning\\F4_LD_AMA\\F4_tons\\fichiers_archives\\SubjectAndVerb\\Tone_Subject_Verb_F4_2012.xml");
...
print XOUT "<TEXT id=\"crdo-NRU_$textname\" xml:lang=\"nru\">\n";
...
while ($line=<VOC>) {
		...
		$nblines++;		
		chomp $det;
		if ($det eq 'x') {       #this is not a determinative compound and no information is written. 
		}
		else {
		# writing the determiner into the file
			print  XOUT "\t<NOTE xml:lang=\"en\" message=\"Object: $det\"/>\n";
		}
		
		$head=<VOC>;# head
		$nblines++;		
		chomp $head;
		if ($head eq 'x') {       #this is not a determinative compound and no information is written. 
		}
		else {
			# writing the head into the file
			print  XOUT "\t<NOTE xml:lang=\"en\" message=\"Verb: $head\"/>\n";
		}		
		...
		if ($input_tones eq 'x') {       #this is not a determinative compound and no information is written. 
			...
			print  XOUT "\t<NOTE xml:lang=\"en\" message=\"Input tones (separated by a space): $input_tones\"/>\n";
		}	
